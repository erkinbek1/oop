...
let filtersPriceFromInp = $("#filters-price-from")
let filtersPriceToInp = $("#filters-price-to")
...
async function getCars() {
  let data = await fetch(`${API}?q=${search.val()}&_page=${page}&_limit=${limit}&price_gte=${filtersPriceFromInp.val()}&price_lte=${filtersPriceToInp.val()}`).then((res) => res.json());
  console.log(data);
  carsList.empty();
  data.forEach((car) => {
    carsList.append(`
  ...
  pagination.html(`<button ${page == 1 ? "disabled" : ""} id="btn-prev">Prev<button/><span>${page}</span><button id="btn-next">Next<button/>`)
}
...
$("body").on("click", ".btn-details", async function (e) {
  ...
  details.html(`<video autoplay loop muted width="100%"><source src=${detailsData.video}></video> <div id="carouselExampleIndicators" class="carousel slide" data-bs-ride="carousel">
